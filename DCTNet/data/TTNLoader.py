...
class TTNData(DatasetBase):
    def __init__(self, slice_id=0, slice_count=1,dist=False, **kwargs):
        ...
        self.src_paths = [os.path.join(src_root,f) for f in os.listdir(src_root) if f.endswith('.png') or f.endswith('.jpg')]
        self.tgt_paths = [os.path.join(tgt_root,f) for f in os.listdir(tgt_root) if f.endswith('.png') or f.endswith('.jpg')]
